...
[`all.sh`](all.sh) 是全仓库离线回归聚合入口，依次运行四套终端环境测试和一套 Multipass
扩展测试；任一失败立即停止。它不持有夹具或断言，中断时把信号转交当前套件并等待清理。
五套测试可以独立运行。
...
[multipass.sh](multipass.sh) 使用 `multipass/cases.py` 检查 VM 参数、安装复用、SSH 信任、
固定提交和冲突保护。用例无网络，使用宿主命令与 VM 替身，不要求安装真实 Multipass，
也不会创建客户机。

服务就绪测试保留真实 Runner 的异常转换，只替换命令执行并使用虚拟
时钟，覆盖 daemon 延迟、旧 CLI 回退、超时和配置错误。临时 Git 仓库验证活跃或遗留
bootstrap 锁会阻止 checkout，并在锁解除后允许继续。

VM 替身验证创建记录先于 launch、
失败保留、同名实例缺失时不重新 launch，以及验收清理同时核对本次记录、宿主状态与客户机
标记；名称冲突、并发竞争和锁占用均断言既有实例与 SSH 文件保持不变。
这些模拟结果不能代替原生 macOS 宿主和真实虚拟机验收。

## 真实虚拟机验收

[multipass-live.sh](multipass-live.sh) 是独立的显式验收入口，不由 `all.sh` 调用。
它需要符合要求的 Apple Silicon Mac 宿主、已解锁加入 ssh-agent 的专用 SSH 身份、
远程可获取的完整提交 SHA 和实际网络；会依次创建 Ubuntu 24.04、26.04 客户机。
准备、运行命令、归属核对与清理边界见
[Multipass 开发机说明](../environments/multipass/README.md#维护与验收)。

## 运行

在仓库根目录按需要运行全量回归、核心套件或扩展套件：
...
入口也可以从任意工作目录按绝对路径调用。用 `/path/to/bash tests/all.sh` 指定聚合器和
套件入口的 Bash；调用核心 Bash 脚本的套件会把所选解释器传给被测脚本。
...
[脚本说明](../scripts/README.md#代码导航与验证)。工具缺失应记录为未验证，不能算通过：
