...
若已创建过的实例被删除，宿主回执和 SSH 信任仍会保留。同名 `create` 会在 launch 前
报出冲突和状态路径；使用新的 `--name` 创建下一台开发机。同名自动重建暂不支持，
不要通过直接清空 known_hosts 或复用旧回执绕过实例身份检查。
...
入口在修改 SSH、更新仓库之前检查客户机 bootstrap 锁；仓库操作和 bootstrap 自身
也会重新检查。仍在运行、PID 缺失或无法核实的锁均保留并报错。
实例停止时先执行 `multipass start <name>`。主机公钥不符时先排查实例是否被替换，
不要关闭 `StrictHostKeyChecking` 或直接清空 known_hosts。
Multipass 安装或升级后的版本及服务探测在 120 秒窗口内重试命令失败；版本不匹配、
版本不达标和不支持的驱动等配置错误立即停止。
...
每版的 `creation.json` 由创建命令在 launch 前独占写入，记录本次的新实例名与 UUID；
即使后续创建失败，也可核对该记录、宿主声明和客户机标记后清理。没有本次记录、
状态或标记不匹配、实例锁仍被占用时，不删除实例。预先存在的实例及状态目录均被拒绝。
自动化调用可通过 `create --creation-record <绝对路径>` 获取同样的创建意图记录：
要求全新的实例名、状态目录和记录文件，父目录必须已存在；预览不会写入记录。
