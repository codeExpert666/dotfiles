...
官方 pkg 安装会在当前终端通过 `sudo -v` 请求管理员认证；密码提示和输入直接由
`sudo` 处理，不写入脚本日志。后续安装保留同一终端会话，通过 `sudo -n installer`
复用认证。需要输入密码时，应从交互终端运行原命令；认证失败后可用相同参数重试，
已通过 SHA-256 校验的安装包缓存会被复用。
